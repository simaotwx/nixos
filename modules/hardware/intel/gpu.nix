{ config, options, lib, pkgs, ... }: {
  ...
  in rec {
    ...
    systemd.services = (lib.optionalAttrs config.customization.graphics.intel.rgbFix {
      intel-rgb-fix = {
    ...
    boot.initrd.systemd.services.intel-rgb-fix =
      lib.mkIf (systemd.services ? intel-rgb-fix) systemd.services.intel-rgb-fix;
