{ lib, config, ... }: {
  ...
  config = {
    ...
    virtualisation.docker.rootless = lib.mkIf config.customization.virtualisation.docker.rootless.enable {
      enable = true;
    ...
    systemd.services.docker.serviceConfig =
      if config.customization.virtualisation.docker.autostart then { Restart = lib.mkForce "no"; } else {};
