{ ... }:
rec {
  concatOutputCombination = { name, artifact, variant ? null, deviceName }:
    if variant == null || variant == "default"
    then "${name}/${artifact}:${deviceName}"
    else "${name}/${artifact}@${variant}:${deviceName}";
  ...
  mkTargetOutputs =
    ...
    let
      ...
      flattenedOutputs = builtins.concatLists (
        builtins.attrValues (
          builtins.mapAttrs (artifact: value:
            ...
            then map (variant: {
              key = concatOutputCombination { inherit name artifact variant deviceName; };
              value = value.${variant};
